...
class RTDETRPredictor(BasePredictor):
    ...
    def postprocess(self, preds, img, orig_imgs):
        ...
        results = []
        for i, bbox in enumerate(bboxes):  # (300, 4)
            # 将 bbox 转换为 [x1, y1, x2, y2]
            bbox = ops.xywh2xyxy(bbox)
            score, cls = scores[i].max(-1, keepdim=True)  # (300, 1)

            # 根据置信度阈值筛选
            idx = score.squeeze(-1) > self.args.conf  # (300, )
            if self.args.classes is not None:  # 类别筛选
                idx = (cls == torch.tensor(self.args.classes, device=cls.device)).any(1) & idx

            # 筛选后的预测结果
            pred = torch.cat([bbox, score, cls], dim=-1)[idx]  # (num_filtered, 6)

            # 获取原始图像
            orig_img = orig_imgs[i]
            oh, ow = orig_img.shape[:2]

            if pred.shape[0] == 0:  # 没有检测框
                # 返回空结果，但保留原图信息
                results.append(
                    Results(orig_img, path=self.batch[0][i], names=self.model.names, boxes=torch.zeros((0, 6))))
                continue

            # 应用 NMS
            boxes, scores, classes = pred[:, :4], pred[:, 4], pred[:, 5]
            keep = torchvision.ops.nms(boxes, scores, self.args.iou)  # NMS
            pred = pred[keep]  # 过滤冗余框
