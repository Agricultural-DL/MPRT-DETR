...
class RTDETRDetectionModel(DetectionModel):
    ...
    def find_closest_indices(self, gt_groups):
        # 初始化结果列表
        closest_indices = []

        # 遍历 gt_groups 中的每个值
        for value in gt_groups:
            if 0 <= value < 100:
                closest_indices.append(0)  # 返回 0
            elif 100 <= value < 200:
                closest_indices.append(1)  # 返回 1
            elif 200 <= value < 300:
                closest_indices.append(2)  # 返回 2
            elif value >= 300:
                closest_indices.append(3)  # 返回 3
            else:
                # 如果值不在任何分段内，可以抛出异常或返回默认值
                raise ValueError(f"Value {value} is out of range for gt_groups.")
    ...
    #         tensor_enhanced = torch.from_numpy(enhanced_image.astype(np.float32) / 255.0).permute(2, 0, 1)
    ...
    def expand_and_clamp_bboxes(self, bboxes, expand_ratio=0.1, selection_ratio=0.2):
        """
        对 YOLO 格式的检测框 (x_center, y_center, w, h) 进行膨胀，并确保边界不越界 [0, 1]。
        随机选取 20% 的检测框进行膨胀。

        参数:
            bboxes (torch.Tensor): 形状为 (N, 4)，每行为 [x_center, y_center, w, h]。
            expand_ratio (float): 膨胀比例，例如 0.1 表示宽度和高度增加 10%。
            selection_ratio (float): 随机选择的比例，默认为 0.2，表示选择 20% 的检测框进行膨胀。

        返回:
            torch.Tensor: 形状为 (N, 4)，膨胀并修正后的检测框。
        """
        # 确保 bboxes 是 tensor 类型
        bboxes = bboxes.to(torch.float32)

        # 生成一个随机选择的布尔索引（20%的概率选择）
        num_boxes = bboxes.size(0)
        random_indices = torch.rand(num_boxes) < selection_ratio

        # 获取原始的 x_center, y_center, w, h
        x_center, y_center, w, h = torch.unbind(bboxes, dim=1)
        ...
        # 修正宽度和高度，确保不超过图像的边界
        w_new = torch.min(w_new, torch.tensor(1.0))
        h_new = torch.min(h_new, torch.tensor(1.0))

        # 计算新的边界框的左上角和右下角
        x_min = torch.max(x_center - w_new / 2, torch.tensor(0.0))
        y_min = torch.max(y_center - h_new / 2, torch.tensor(0.0))
        x_max = torch.min(x_center + w_new / 2, torch.tensor(1.0))
        y_max = torch.min(y_center + h_new / 2, torch.tensor(1.0))
        ...
        # 将未选择膨胀的框保留原样
        x_center_new[~random_indices] = x_center[~random_indices]
        y_center_new[~random_indices] = y_center[~random_indices]
        w_final[~random_indices] = w[~random_indices]
        h_final[~random_indices] = h[~random_indices]

        # 合并结果
        expanded_bboxes = torch.stack([x_center_new, y_center_new, w_final, h_final], dim=1)
    ...
    def loss(self, batch, epoch, preds=None):
        ...
        #img = self.enhance_contrast_batch(img).to(device)
        gt_groups = [(batch_idx == i).sum().item() for i in range(bs)]
        #if self.training:
        batch_boxes = self.expand_and_clamp_bboxes(batch['bboxes'],0.1,0.2)
        ...
        dec_bboxes, dec_scores, enc_bboxes, enc_scores, dn_meta, pre_count= preds if self.training else preds[1]
        if dn_meta is None:
        ...
        loss = self.criterion((dec_bboxes, dec_scores),
                              ...
                              dn_scores=dn_scores,
                              dn_meta=dn_meta)
        #dynamic_query_list = [300, 600, 900, 1500]
        num_select = self.find_closest_indices(gt_groups)
        num_select_tensor = torch.tensor(num_select).to(device=img.device)
        #print('epoch:',epoch)
        #if epoch<5:
        count_loss = nn.CrossEntropyLoss()
        # # 计算分类损失
        loss['loss_count'] = count_loss(pre_count, num_select_tensor)*0.1
        #     # # 创建 CrossEntropyLoss 实例
        #     #print('True')
        #     for k in loss.keys():
        #         loss[k] *= 0

        #print('loss[loss_count]:',loss['loss_count'])
    ...
    def predict(self, x, profile=False, visualize=False, batch=None, augment=False, embed=None):
        ...
        for m in self.model[:-1]:  # except the head part
            ...
            if hasattr(m, 'backbone'):
                x = m(x)
                #print('11111111111111111',len(x))


                if len(x) != 5:  # 0 - 5
                    x.insert(0, None)

                # 添加了iAT头，所以index需要+1,否则self.save和index或不等,y中只会保留三层
                for index, i in enumerate(x):
                    if index in self.save:
                ...
                #print('m',m)
                x = m(x)  # runexpand_and_clamp_bboxes
                y.append(x if m.i in self.save else None)  # save output
            if visualize:
                feature_visualization(x, m.type, m.i, save_dir=visualize)
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
                 ...
                 nn.Conv2d, ADown,nn.Conv2d,RepC3_WTConv,EMA_attentionC3):
            c1, c2 = ch[f], args[0]
            ...
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3,
                     RepC3_WTConv,EMA_attentionC3):
                args.insert(2, n)  # number of repeats
                n = 1
                             
        elif m in {Dy_Sample, EMA, IAT}:
            c2 = ch[f]
            args = [c2, *args]
        #
        # elif m is multiply:
        ...
        elif m in {FasterNet_A,FasterNet_A1}:
        ...
        #print('mmmmmmmmmmmmmmmmmm',m_.parameters())
