...
class CBlock_ln(nn.Module):
    def __init__(self, dim, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        super().__init__()
        self.pos_embed = nn.Conv2d(dim, dim, 3, padding=1, groups=dim)
        #self.RFB = RFB_modified(dim, 4)
        #self.norm1 = Aff_channel(dim)
        self.norm1 = norm_layer(dim)
        self.conv1 = nn.Conv2d(dim, dim, 1)
        self.conv2 = nn.Conv2d(dim, dim, 1)
        #self.attn = nn.Conv2d(dim, dim, 5, padding=2, groups=dim)
        self.attn = WTConv2d(dim, dim, 5)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
    ...
    def forward(self, x):
        ...
        #x = x + self.drop_path(self.gamma_1 * self.RFB(norm_x))
        norm_x = x.flatten(2).transpose(1, 2)
...
class Local_pred(nn.Module):
    def __init__(self, dim=4, number=4, type='ccc'):
        ...
        if type == 'ccc':
            ...
            blocks1 = [CBlock_ln(4, drop_path=0.01), CBlock_ln(4, drop_path=0.05), CBlock_ln(4, drop_path=0.1)]
            blocks2 = [CBlock_ln(4, drop_path=0.01), CBlock_ln(4, drop_path=0.05), CBlock_ln(4, drop_path=0.1)]
        elif type == 'ttt':
...
class Local_pred_S(nn.Module):
    def __init__(self, in_dim=3, dim=4, number=4, type='ccc'):
...
class IAT(nn.Module):
    ...
    def forward(self, img_low):
        ...
        #torchvision.utils.save_image(mul, 'mul.png')
        #torchvision.utils.save_image(add, 'add.png')
        img_high = (img_low.mul(mul)).add(add)
        #torchvision.utils.save_image(img_high, 'img_high1.png')
        if not self.with_global:
            ...
            #torchvision.utils.save_image(img_high, 'img_high2.png')
            # torchvision.utils.save_image(gamma, 'gamma.png')
